...
- **読み上げは注音に合わせる**。見出し語は漢字ではなく読み（第2フィールド）を鳴らし、
  例文は `part2.html` の `YOMI`（読みが割れる語の一覧）に載っている語だけ仮名に開いてから
  音声エンジンに渡す。「十分」を「じゅうぶん」と読まれるような食い違いを防ぐためで、
  そういう語を新しく足したときは `YOMI` にも足す
